from algorithms.EMAC.Data import Data
import algorithms.EMAC.GraphUtils
import numpy as np
import algorithms.EMAC.Utils
...
class SCAN:
    ...
    def calc_init_score(self):
        ...
        sorted_index = algorithms.EMAC.Utils.get_sorted_index(init_value_outlierness)
    ...
    def construct_network(self, init_score_eta):
        ...
        k = 2
        while True:
            spectral = SpectralClustering(n_clusters=k, gamma=1, assign_labels="discretize").\
        ...
        graph = algorithms.EMAC.Utils.generate_graph(adjacency_matrix)
        return adjacency_matrix, graph

    def learn_embeddings(self, graph, adjacency_matrix):
        values_num = self.data.values_num
        G = algorithms.EMAC.GraphUtils.Graph(graph, adjacency_matrix)
        G.preprocess_transition_probs()
        walks = G.simulate_walks(self.num_walks, self.walk_length)
        model = Word2Vec(walks, vector_size=self.dimensions, window=self.window_size, min_count=0, sg=1, workers=8, epochs=5)
    ...
    def value_scoring(self, value_coupling, init_score_eta):
        values_num = self.data.values_num
        last_rank = algorithms.EMAC.Utils.get_sorted_index(init_score_eta)
        size = int(values_num * self.alpha)
        ...
        while True:
            ...
            rank = algorithms.EMAC.Utils.get_sorted_index(value_score)
            iter_count += 1
