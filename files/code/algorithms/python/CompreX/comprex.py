...
from algorithms.CompreX.partition import Partition
from algorithms.CompreX.feature_set import FeatureSet
from algorithms.CompreX.code_table import CT
...
class CompreX(BaseEstimator, ClassifierMixin):
    ...
    def score(self, X, y=None):
        # feature, values, scores_by_feature
        result = [(ct.feature_set.name.replace('FS__', ''), ct.patterns, ct.calculate_patterns_length()) for ct in self.partition_init.code_tables]
        # Mount relation table
        table = {}
        for r in result:
            table[r[0]] = {}
            for score, value in zip(r[1], r[2]):
                table[r[0]][score] = value

        # Calcule total score for rows
        for row in X:
            score = 0
