...
def graph_ranks(avranks, names, p_values, cd=None, cdmethod=None, lowv=None, highv=None,
    ...
    height = (cline + ((k + 1) / 2) * 0.2 + minnotsignificant) + 0.7
    ...
    def text(x, y, s, *args, **kwargs):
        ax.text(wf * x, hf * y, s, color='black' if s not in dict_color else dict_color[s], *args, **kwargs)
    ...
    for a in range(lowv, highv + 1):
        # Imprimir apenas os índices pares
        if a % 2 == 0:
...
def wilcoxon_holm(alpha=0.01, df_perf=None):
    ...
    if False and friedman_p_value >= alpha:
...
#df_perf = pd.read_csv(r'cd-diagram-master\example.csv', index_col=False)
